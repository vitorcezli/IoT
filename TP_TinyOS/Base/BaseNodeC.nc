...
implementation{
    ...
    event void Timer0.fired() {
        id_req_atual++;
        if (!_isBusy) {
            reqTopMsg_t* m=(reqTopMsg_t*)(call Packet.getPayload(&_packet, sizeof(reqTopMsg_t)));
            if (m == NULL) {
                return;
            }
            ...
            if (call AMSend.send(AM_BROADCAST_ADDR, &_packet, sizeof(reqTopMsg_t)) == SUCCESS) {
                _isBusy = TRUE;
            }
        }
    }
    ...
    event void Timer1.fired() {
        id_req_atual++;
        if (!_isBusy) {
            reqDadoMsg_t* m = (reqDadoMsg_t*)(call Packet.getPayload(& _packet, sizeof(reqDadoMsg_t)));
            if (m == NULL) {
                return;
            }
            m->src = (nx_uint8_t)TOS_NODE_ID;
            m->tipo=AM_REQ_DADOS;
            m->id_req=id_req_atual;
            if (call AMSend.send(AM_BROADCAST_ADDR, &_packet, sizeof(reqDadoMsg_t)) == SUCCESS) {
                _isBusy = TRUE;
            }
        }
    }
